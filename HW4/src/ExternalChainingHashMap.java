...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if (table[hashValue] == null) {
            table[hashValue] = new ExternalChainingMapEntry<K, V>(key, value);
            size++;
            return null;
        } else {
            ExternalChainingMapEntry<K, V> check = table[hashValue];
            do {
            ...
            ExternalChainingMapEntry<K, V> temp = new ExternalChainingMapEntry(key, value, table[hashValue]);
    ...
    public V remove(K key) {
        ExternalChainingMapEntry<K, V> t;
        int counter = 0;
        if (key == null) {
            throw new IllegalArgumentException("key is null");
        }
        ...
        if (table[hashValue] == null) {
            ...
            ExternalChainingMapEntry<K, V> searchV = table[hashValue];
            do {
                if (searchV.getKey().equals(key)) {
                    if (counter == 0) {
                        if (searchV.getNext() == null) {

                            table[hashValue] = null;
                        } else {
                            table[hashValue] = searchV.getNext();
                        }
                    } else {
                        t = table[hashValue];
                        ...
                        if (searchV.getNext() == null) {
                            ...
                            t.setNext(searchV.getNext());
                        }

                    }

                    size--;
                    return searchV.getValue();
                }
                if (searchV.getNext() != null) {
                    searchV = searchV.getNext();
                }
                counter++;
            } while (searchV.getNext() != null);
            if (searchV.getKey().equals(key)) {
                if (counter == 0) {
                    if (searchV.getNext() == null) {
    ...
    public void resizeBackingTable(int length) {
        ...
        ExternalChainingMapEntry<K, V> resizeTable;
        size = 0;
        ExternalChainingMapEntry<K, V>[] temptable = table;
        table = (ExternalChainingMapEntry<K, V>[]) new ExternalChainingMapEntry[length];
        for (ExternalChainingMapEntry<K, V> item: temptable) {
            size = 0;
            if (item != null) {
                put(item.getKey(), item.getValue());
                resizeTable = item;
                while (resizeTable.getNext() != null) {
                    put(resizeTable.getKey(), resizeTable.getValue());
                    resizeTable = resizeTable.getNext();
                }
                put(resizeTable.getKey(), resizeTable.getValue());
            }
        }
